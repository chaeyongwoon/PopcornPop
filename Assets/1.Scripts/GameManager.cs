...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] Button[] MPS_Button;
    ...
    [FoldoutGroup("Sound")] public AudioSource[] _audio;
    ...
    public bool isStore_Hide = false;
    public float Vibe_Interval = 0.1f;
    public bool Vibe_isReady = true;

    public UnityEngine.UI.Text touchText;
    ...
    private void Awake()
    {
        ...
        RV_Text = new Text[4];
        MPS_Text = new Text[6];
        MPS_value = new double[6];
        MPS_Temp = new double[6];
        Stage_Income = new double[6];
        MPS_Button = new Button[6];
    ...
    private void Start()
    {
        ...
        if (isStore_Hide)
        {
            Store_Button.gameObject.SetActive(false);
        }
        ...
        if (_noAdsOn == 0)
        {
            Store_ads.SetActive(true);

            for (int i = 0; i <= Current_Max_Stage_Level; i++)
            {
                Stage_Group[i].GetComponent<StageManager>().stage_ads.SetActive(true);
            }
            // true
        }
        else
        {
            Store_ads.SetActive(false);

            for (int i = 0; i <= Current_Max_Stage_Level; i++)
            {
                Stage_Group[i].GetComponent<StageManager>().stage_ads.SetActive(false);
            }
            // false
        }
    }
    ...
    void SetStage()
    {
        ...
        //RV_Button_Group[2].transform.GetChild(1).gameObject.SetActive();
        RV_Super_Fever_time = 0;
        Stage_Income[Current_Stage_Level] = Current_Up_Income;

        for (int i = 0; i < MPS_Button.Length; i++)
        {
            MPS_Button[i].interactable = true;
            MPS_Button[i].GetComponent<Image>().SetNativeSize();
        ...
        //if (isStore_Hide == false)
        //{
        //    _storemanager.InitPool();
        //}

        if (Current_Stage_Level < 3)
        {
            //MPS_Button[0].gameObject.SetActive(true);
            //MPS_Button[1].gameObject.SetActive(true);
            //MPS_Button[2].gameObject.SetActive(true);

            MPS_Button[3].gameObject.SetActive(false);
            MPS_Button[4].gameObject.SetActive(false);
            MPS_Button[5].gameObject.SetActive(false);
        }
        else
        {
            MPS_Button[0].gameObject.SetActive(false);
            MPS_Button[1].gameObject.SetActive(false);
            MPS_Button[2].gameObject.SetActive(false);

            //MPS_Button[3].gameObject.SetActive(true);
            //MPS_Button[4].gameObject.SetActive(true);
            //MPS_Button[5].gameObject.SetActive(true);
        }
    ...
    IEnumerator Cor_Update()
    {
        ...
        while (true)
        {
            ...
            RV_Text[3].text = RV_GoldPop_bool ? string.Format("00:{0:N0}", RV_GoldPop_time) : "Corn Price X 3";
            ...
            MPS_Text[3].text = string.Format("{0}/s", ToCurrencyString(MPS_value[3]));
            MPS_Text[4].text = string.Format("{0}/s", ToCurrencyString(MPS_value[4]));
            MPS_Text[5].text = string.Format("{0}/s", ToCurrencyString(MPS_value[5]));
    ...
    public void ManagerAddMoney(int _type = 0)
    {
        switch (_type)
        {
            case 0:
    ...
    void Check_Button()
    {
        ...
        if (Current_Popcorn_Level < Current_Popcorn_Max_Level)
        {
            ...
            else
            {
                ...
                if (PlayerPrefs.GetInt("NoAds", 0) == 0)
                {
                    Upgrade_Button[0].transform.GetChild(2).gameObject.SetActive(true); // rv 이미지
                }
                else
                {
                    Upgrade_Button[0].transform.GetChild(2).gameObject.SetActive(false);
    ...
    public void Popcorn_Upgrade()
    {
        ...
        if (Current_Popcorn_Level < Current_Popcorn_Max_Level)
        {
            ...
            else if (PlayerPrefs.GetInt("NoAds", 0) == 0)
            {
                AdsManager.ShowRewarded(() =>
                {
                    ...
                    EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Upgrade_RV", "AddCorn" } });
                });
            }
        ...
        if (Current_Income_Level < Current_Income_Max_Level)
        {
            ...
            else if (PlayerPrefs.GetInt("NoAds", 0) == 0)
            {
                AdsManager.ShowRewarded(() =>
                {
                    ...
                    EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Upgrade_RV", "Income" } });
                });
            }
        }
    }
    public void Obj_Upgrade()
    {
        ...
        if (Current_Object_Level < Current_Obj_Max_Level)
        {
            if (Money > Current_Object_Base_Price * MathF.Pow(Current_Object_Upgrade_Scope, Current_Object_Level))
            {
                ...
                if (Current_Object_Level == 4 && Current_Stage_Level == 0)
                {
                    CustomReviewManager.instance.StoreReview();
                    //Debug.Log("send review");
                }
    ...
    void SetButton()
    {
        ...
        MPS_Button[3] = MPS_Panel.transform.GetChild(3).GetComponent<Button>();
        MPS_Button[4] = MPS_Panel.transform.GetChild(4).GetComponent<Button>();
        MPS_Button[5] = MPS_Panel.transform.GetChild(5).GetComponent<Button>();
        ...
        MPS_Text[3] = MPS_Button[3].transform.GetChild(0).GetComponent<Text>();
        MPS_Text[4] = MPS_Button[4].transform.GetChild(0).GetComponent<Text>();
        MPS_Text[5] = MPS_Button[5].transform.GetChild(0).GetComponent<Text>();
    ...
    void RV_Spawn_Dobule() // double spawn count
    {
        ...
        DOTween.Sequence().AppendCallback(() =>
        {
            ...
            EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Base_RV", "Spawn" } });
    ...
    void RV_Income_Double() // double income
    {
        ...
        DOTween.Sequence().AppendCallback(() =>
        {
            ...
            EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Base_RV", "Income" } });
    ...
    void RV_Super_Fever() // super spawn object and open 4 door
    {
        ...
        DOTween.Sequence().AppendCallback(() =>
        {
            ...
            EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Base_RV", "AutoClick" } });
    ...
    void RV_GoldPop()
    {
        ...
        DOTween.Sequence().AppendCallback(() =>
        {
            ...
            EventTracker.LogCustomEvent("RV", new Dictionary<string, string> { { "Base_RV", "GoldPop" } });
    ...
    {
        if (isVibe && Vibe_isReady)
        {
            Debug.Log("Vibe");
            Vibe_isReady = false;
            DOTween.Sequence().AppendInterval(Vibe_Interval)
                .OnComplete(() => Vibe_isReady = true);
            switch (_num)
            {
                case 0:
    ...
    public void Sound(int _num, int audio_num = 0)
    {
        if (isSound)
        {
            switch (audio_num)
            {
                case 0:
                    _audio[0].clip = _clips[_num];
                    _audio[0].Play();
                    break;

                case 1:
                    _audio[1].clip = _clips[_num];
                    _audio[1].Play();
                    break;

                case 2:
                    if (_audio[2].isPlaying == false)
                    {
                        _audio[2].clip = _clips[_num];
                        _audio[2].Play();
                    }
                    break;
    ...
    void RV_Button_OnOff()
    {
        ...
        if (isRV_Visible)
        {
            ...
            RV_Button_Group[RV_Current_num].transform.GetChild(1).GetComponent<Image>().fillAmount = 0f;
        }
        else
        {
            RV_Current_Wait_Interval = 0;
    ...
    public void IAP_NoAdsPurchase()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            DOTween.Sequence(RV_Button_Group[i].GetComponent<RectTransform>().DOLocalMoveX(RV_x, 0.5f).SetEase(Ease.Linear));
            RV_Button_Group[i].transform.GetChild(1).GetComponent<Image>().fillAmount = 0f;
        }
        RV_Button_Group[0].onClick.RemoveAllListeners();
        RV_Button_Group[1].onClick.RemoveAllListeners();
        RV_Button_Group[2].onClick.RemoveAllListeners();
        RV_Button_Group[3].onClick.RemoveAllListeners();
        ...
        RV_Button_Group[3].onClick.AddListener(() => RV_GoldPop());

        for (int i = 0; i < 4; i++)
        {
            RV_Button_Group[i].transform.GetChild(2).gameObject.SetActive(false);
        }
    ...
    public void StoreCam_OnOff(bool isOn)
    {
        ...
        if (isOn)
        {
            AdsManager.ChangeAdvertyCamera(Store_Cam.GetComponent<Camera>());

        }
        else
        {
            AdsManager.ChangeAdvertyCamera(Camera.main);
        }
